...
//	ПолеВвода.ПутьКДанным = "РР_КонтактноеЛицо";
//	
//КонецПроцедуры   

//Процедура ДобавитьПоляРассчетаСкидки(Форма)   

//	//Добавление ГруппыСкидки на форму
...
//	ПолеВвода.Вид = ВидПоляФормы.ПолеВвода;
//	ПолеВвода.ПутьКДанным = "РР_СогласованнаяСкидка";
//	
//	//Добавление Команды на форму
//	КомандаФормы = Форма.Команды.Добавить("ПерессчитатьТаблицу");
//	КомандаФормы.Действие = "РР_ПерессчитатьТаблицу"; 
//	КомандаФормы.Заголовок = "Перессчитать таблицу";
//	
//	НовыйЭлемент = Форма.Элементы.добавить("ПерессчитатьТаблицу", Тип("КнопкаФормы"), Форма.Элементы.ГруппаСкидка);
//	НовыйЭлемент.ИмяКоманды = "РР_ПерессчитатьТаблицу";
...
Процедура ПриСозданииНаСервере(Форма, Знач ПараметрыРазмещения = Неопределено) Экспорт
	...
	// ++ Романов Задача _______ 01.09.2024
